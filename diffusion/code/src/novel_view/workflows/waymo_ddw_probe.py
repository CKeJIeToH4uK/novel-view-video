"""Complete the four-variant historical DDW probe before its verdict."""
...
def run_v1(job: ResolvedJob, runtime: RuntimeContext) -> int:
    ...
    from novel_view.preparation.waymo_ddw.legacy.record import write_probe_record
    from novel_view.preparation.waymo_ddw.legacy.v2_canary import measure_waymo_ddw_v3_probe

    spec = parse_job(job)
    result = runtime.attempt_root / "result.json"
    with open_legacy_ddw_measurement(spec, runtime) as measurement:
        evidence = measure_waymo_ddw_v3_probe(
            measurement.source, spec.recipe.axis, measurement.warp,
            measurement.reference, measurement.logs,
        )
        write_probe_record(result, spec.input.depth_selection, measurement.depth, evidence)
    print(result)
