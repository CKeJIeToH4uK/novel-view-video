...
Полные исторические пары класса A для основной метрики.
Порядок пар и source/target токенов задаёт
`selections/euvs/euvs-setting1-primary35.yaml`.

1. Выполнить `source-views/run.yaml` через корневой `./distil3d` с
   подходящим профилем: задание `euvs-primary35-source-views-v1`
...
3. Выбрать `run.yaml` для `euvs_generation/v1` с авторегрессией
   ...
   `euvs-primary35-source-views-v1/replace-run/attempts/replace-attempt`.
...
`euvs-primary35-autoregressive-v1/<run>/attempts/<attempt>` либо
`euvs-primary35-source-reseed-v2/<run>/attempts/<attempt>`.
