...
Полные исторические комбинации для исследования отказов и чувствительности.
Порядок пар и source/target токенов задаёт
`selections/euvs/euvs-setting1-raw29.yaml`.

1. Выполнить `source-views/run.yaml` через корневой `./distil3d` с
   подходящим профилем: задание `euvs-raw29-source-views-v1`
...
3. Выбрать `run.yaml` для `euvs_generation/v1` с авторегрессией
   ...
   `euvs-raw29-source-views-v1/replace-run/attempts/replace-attempt`.
...
`euvs-raw29-autoregressive-v1/<run>/attempts/<attempt>` либо
`euvs-raw29-source-reseed-v2/<run>/attempts/<attempt>`.
