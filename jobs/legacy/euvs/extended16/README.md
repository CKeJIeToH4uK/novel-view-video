...
Дополнительные исторические пары классов B/C; обрезанные target-последовательности уже заданы выборкой.
Порядок пар и source/target токенов задаёт
`selections/euvs/euvs-setting1-extended16.yaml`.

1. Выполнить `source-views/run.yaml` через корневой `./distil3d` с
   подходящим профилем: задание `euvs-extended16-source-views-v1`
...
3. Выбрать `run.yaml` для `euvs_generation/v1` с авторегрессией
   ...
   `euvs-extended16-source-views-v1/replace-run/attempts/replace-attempt`.
...
`euvs-extended16-autoregressive-v1/<run>/attempts/<attempt>` либо
`euvs-extended16-source-reseed-v2/<run>/attempts/<attempt>`.
